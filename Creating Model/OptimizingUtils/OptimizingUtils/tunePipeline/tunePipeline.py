...
def get_imputer(strategy, fill_value=None):
    ...
    This function creates and returns an instance of `SimpleImputer`, `KNNImputer`, or `IterativeImputer`
    based on the given strategy.

    Parameters:
        strategy (str): The imputation strategy. Can be one of 'mean', 'median', 
                        'most_frequent', 'constant', 'knn', or 'iterative'.
        fill_value (optional): The value to use for imputing missing values when the 
                               strategy is 'constant'. If not provided, the default fill 
                               value for 'constant' strategy is used.

    Returns:
        Imputer: An instance of `SimpleImputer`, `KNNImputer`, or `IterativeImputer`
                 configured with the specified strategy and fill value.
    """
    ...
    try:
        with mlflow.start_run(nested=True) as run:
            ...
            numerical_imputer_strategy = trial.suggest_categorical(
                'numerical_imputer_strategy', ['mean', 'median', 'most_frequent', 'constant', 'knn', 'iterative']
            )
