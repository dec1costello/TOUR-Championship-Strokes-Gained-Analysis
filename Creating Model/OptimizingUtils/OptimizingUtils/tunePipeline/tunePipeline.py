...
def objective(trial, model_class, model_param_suggestion, is_stacking=False, base_models=None, categorical_cols=None, numerical_cols=None, X=None, y=None):
    """
...
def log_and_print_best_trial(study):
    """

    Log and print the best trial's information from an Optuna study.

    This function retrieves the best trial from the given Optuna study, logs its value
    and parameters to MLflow, and prints the best trial's value and parameters to the console.

    Parameters:
        study (optuna.study.Study): The Optuna study object containing the trials.

    Returns:
        optuna.trial.FrozenTrial: The best trial from the study.    
    """
    # Get the best trial
...
#change title to 'register_best_model_to_production
def register_best_model(best_trial):
    """
    ...
    Parameters:
        best_trial (optuna.trial.FrozenTrial): The best trial containing user attributes with MLflow run details.
    ...
    run_name = best_trial.user_attrs["mlflow_name"]
    artifact_path = best_trial.user_attrs["artifact_path"]
    artifact_uri = f"runs:/{run_id}/{artifact_path}"

    # Load the artifact from MLflow
    local_path = mlflow.artifacts.download_artifacts(artifact_uri)
    with bz2.BZ2File(local_path, 'rb') as f:
        best_model = joblib.load(f)

    # Initialize MLflow client
    client = MlflowClient()

    # Ensure the registered model exists, else create it
    try:
        client.get_registered_model(run_name)
    except mlflow.exceptions.RestException:
        client.create_registered_model(run_name)
    ...
    # Optionally, transition the model to the "Production" stage
